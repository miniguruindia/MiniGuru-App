...
exports.reviewVideoFile = reviewVideoFile;
...
async function waitForRpmSlot() {
    ...
    if (requestTimestamps.length >= RPM_LIMIT) {
        const waitMs = 60000 - (now - requestTimestamps[0]) + 250;
...
async function checkAndIncrementDailyQuota() {
    ...
    try {
        const existing = await prismaClient_1.default.siteContent.findUnique({ where: { key: DAILY_QUOTA_KEY } });
        let data = existing?.value ?? { date: today, count: 0 };
        if (data.date !== today) {
 ...
 */
async function reviewVideoFile(localFilePath, mimeType) {
    ...
    try {
        ...
        let attempts = 0;
        while ((!file.state || file.state === genai_1.FileState.PROCESSING) && attempts < MAX_POLL_ATTEMPTS) {
        ...
        if (file.state === genai_1.FileState.FAILED) {
        ...
        const videoPart = (0, genai_1.createPartFromUri)(file.uri, file.mimeType);
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                response = await ai.models.generateContent({
                    model: MODEL,
                    contents: (0, genai_1.createUserContent)([videoPart, SYSTEM_PROMPT]),
                });
                break;
            }
            catch (err) {
