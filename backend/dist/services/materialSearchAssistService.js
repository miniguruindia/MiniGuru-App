...
exports.verifyImageMatch = verifyImageMatch;
 ...
 */
async function verifyImageMatch(materialName, candidateTitle, candidateImageUrl) {
    const NEUTRAL = { confidence: 0.5, note: 'Not verified — check the photo yourself.' };
    const ai = getClient();
    if (!ai || !candidateImageUrl)
        return NEUTRAL;
    try {
        ...
        const parts = [prompt, { inlineData: { mimeType: contentType, data: base64 } }];
        const response = await ai.models.generateContent({
            model: MODEL,
            contents: (0, genai_1.createUserContent)(parts),
        });
