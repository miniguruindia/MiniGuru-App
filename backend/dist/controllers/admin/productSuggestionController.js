...
exports.approveProductSuggestion = exports.updateProductSuggestion = exports.listProductSuggestions = void 0;
...
const approveProductSuggestion = async (req, res) => {
    ...
    const suggestion = await prismaClient_1.default.productSuggestion.findUnique({ where: { id } });
    if (!suggestion)
        return res.status(404).json({ error: 'Suggestion not found.' });
    ...
    const goinsPrice = typeof overrides.goinsPrice === 'number'
    ...
    try {
        const material = await prismaClient_1.default.material.create({
            data: {
                ...
                amazonASIN: asin,
                amazonUrl: asin ? (0, amazonProductService_1.buildAffiliateUrl)(asin) : undefined,
                showInShop: true,
                showInPlanning: true,
            },
        });
        const updated = await prismaClient_1.default.productSuggestion.update({
            where: { id },
            data: {
                status: 'added',
        ...
        return res.status(500).json({ error: `Could not create material: ${error.message}` });
    }
};
exports.approveProductSuggestion = approveProductSuggestion;
