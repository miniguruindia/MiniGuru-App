...
async function buildUniqueChildEmail(name) {
...
router.put('/schools/:id', async (req, res) => {
    try {
        ...
        if ('phoneNumber' in body)
            userData.phoneNumber = body.phoneNumber || null;
        if ('email' in body && body.email && body.email !== existing.email) {
            const taken = await prismaClient_1.default.user.findUnique({ where: { email: body.email } });
            if (taken)
...
router.get('/schools/:id/children', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/schools/:id/children', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const guardian = await prismaClient_1.default.user.findUnique({ where: { id } });
        if (!guardian || !guardian.isMentor) {
            return res.status(404).json({ message: 'School/T-LAB account not found' });
        }
        const finalPin = pin && /^\d{4}$/.test(String(pin)) ? String(pin) : generatePin();
        const pinHash = await bcryptjs_1.default.hash(finalPin, 10);
...
router.put('/children/:childId', async (req, res) => {
    try {
        ...
        const childData = {};
        if ('name' in body && body.name)
            childData.name = body.name;
        if ('age' in body && body.age)
            childData.age = Number(body.age);
        if ('grade' in body)
            childData.grade = body.grade || null;
        if (Object.keys(childData).length > 0) {
            await prismaClient_1.default.childProfile.update({ where: { id: childId }, data: childData });
        }
        ...
        if (child.linkedUserId) {
            ...
            if (Object.keys(userData).length > 0) {
                updatedLogin = await prismaClient_1.default.user.update({
...
router.post('/children/:childId/reset-password', async (req, res) => {
    try {
        const { childId } = req.params;
        const child = await prismaClient_1.default.childProfile.findUnique({ where: { id: childId } });
        ...
        const user = await prismaClient_1.default.user.findUnique({ where: { id: child.linkedUserId } });
        if (!user)
            return res.status(404).json({ message: 'Linked login account not found' });
        const plainPassword = generatePassword();
        const passwordHash = await bcryptjs_1.default.hash(plainPassword, 10);
        await prismaClient_1.default.user.update({ where: { id: child.linkedUserId }, data: { passwordHash } });
        return res.json({
            message: 'Password reset',
            credentials: { email: user.email, password: plainPassword },
        });
...
router.post('/children/:childId/reset-pin', async (req, res) => {
    try {
        ...
        await prismaClient_1.default.childProfile.update({ where: { id: childId }, data: { pinHash } });
...
router.delete('/children/:childId', async (req, res) => {
    try {
        ...
        if (!child)
            return res.status(404).json({ message: 'Student not found' });
        await prismaClient_1.default.childProfile.update({ where: { id: childId }, data: { isActive: false } });
