...
router.get('/:key', async (req, res) => {
    try {
        ...
        const record = await prismaClient_1.default.siteContent.findUnique({ where: { key } });
        if (record)
            return res.json({ key, value: record.value, updatedAt: record.updatedAt });
        if (exports.DEFAULTS[key])
            return res.json({ key, value: exports.DEFAULTS[key], updatedAt: null });
        return res.status(404).json({ message: `Content section '${key}' not found` });
    }
    catch (error) {
        logger_1.default.error(`GET /cms/${req.params.key}: ${error.message}`);
        return res.status(500).json({ message: 'Failed to fetch content' });
    }
});
// Admin PUT (upsert)
router.put('/:key', authMiddleware_1.authenticateToken, authMiddleware_1.authorizeAdmin, async (req, res) => {
    try {
        const { key } = req.params;
        const { value } = req.body;
        if (value === undefined)
            return res.status(400).json({ message: 'value is required' });
        const record = await prismaClient_1.default.siteContent.upsert({
            where: { key },
            update: { value, updatedBy: req.user?.userId },
            create: { key, value, updatedBy: req.user?.userId },
        });
        logger_1.default.info(`CMS '${key}' updated by admin ${req.user?.userId}`);
        return res.json({ success: true, key, updatedAt: record.updatedAt });
    }
    catch (error) {
        logger_1.default.error(`PUT /cms/${req.params.key}: ${error.message}`);
        return res.status(500).json({ message: 'Failed to save content' });
    }
});
// Admin GET all sections overview
router.get('/', authMiddleware_1.authenticateToken, authMiddleware_1.authorizeAdmin, async (req, res) => {
    try {
        const all = await prismaClient_1.default.siteContent.findMany({
            select: { key: true, updatedAt: true }
        });
        return res.json({ seeded: all.map((r) => r.key), defaults: Object.keys(exports.DEFAULTS) });
    }
    catch (error) {
