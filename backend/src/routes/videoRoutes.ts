...
router.get('/:videoId/materials', async (req: any, res: any) => {
  try {
    ...
    if (!project) {
      ...
      const matched = allProjects.find((p: any) =>
        p.video?.url && p.video.url.includes(videoId)
      );
    ...
    const enriched = await Promise.all(
      (project.materials as Array<{ productId: string; name?: string | null; quantity: number }>)
        .map(async (pm) => {
          let mat: any = null;
          try {
            mat = await (prisma as any).material.findFirst({
              where: {
                OR: [
                  { id: pm.productId },
