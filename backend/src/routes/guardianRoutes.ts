...
router.get('/children', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const linkedIds = children.map((c) => c.linkedUserId).filter(Boolean) as string[];
    const realScores = linkedIds.length
      ? await prisma.user.findMany({ where: { id: { in: linkedIds } }, select: { id: true, score: true } })
      : [];
...
router.get('/children/projects', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    ...
    const linkedUserIds = children.map((c) => c.linkedUserId).filter(Boolean) as string[];
    if (linkedUserIds.length === 0) return res.json([]);
...
router.post('/send-credentials', authenticateToken, async (req: any, res: Response) => {
  try {
