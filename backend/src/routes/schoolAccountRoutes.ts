...
// forward.
router.post('/children/:childId/create-login', async (req: Request, res: Response) => {
  try {
    const { childId } = req.params;
    const child = await prisma.childProfile.findUnique({ where: { id: childId } });
    if (!child) return res.status(404).json({ message: 'Student not found' });
...
router.get('/schools/:id/leaderboard', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const children = await prisma.childProfile.findMany({
      where: { guardianId: id, isActive: true },
    ...
    const linkedIds = children.map((c: any) => c.linkedUserId).filter(Boolean) as string[];
    if (linkedIds.length === 0) return res.json({ leaderboard: [] });

    const users = await prisma.user.findMany({
      where: { id: { in: linkedIds } },
      select: { id: true, score: true },
    });
    const scoreMap = new Map<string, number>(users.map((u: any) => [u.id, u.score]));

    const leaderboard = children
      .filter((c: any) => c.linkedUserId && scoreMap.has(c.linkedUserId))
      .map((c: any) => ({
        childId: c.id,
        name: c.name,
...
router.get('/schools/:id/projects', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const children = await prisma.childProfile.findMany({
      where: { guardianId: id, isActive: true },
    ...
    const linkedIds = children.map((c: any) => c.linkedUserId).filter(Boolean) as string[];
    if (linkedIds.length === 0) return res.json({ projects: [] });

    const nameMap = new Map<string, string>(
      children.filter((c: any) => c.linkedUserId).map((c: any) => [c.linkedUserId, c.name])
    );
