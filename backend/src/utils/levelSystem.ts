...
export interface LevelInfo {
  ...
  emoji: string;
  minScore: number;
  maxScore: number | null; // null = open-ended top tier
}
...
export interface LevelProgress {
  ...
  nextLevelAt: number | null;   // null once at the open-ended top tier
  goinsToNextLevel: number | null;
  progressToNext: number;       // 0.0–1.0, always 1.0 at the top tier
}
...
export function getLevelForScore(score: number): LevelProgress {
  const safeScore = Math.max(0, score);
  const current = LEVELS.find((l) => safeScore >= l.minScore && (l.maxScore === null || safeScore <= l.maxScore))
