...
export function ProductForm({ product, onSubmit, onCancel }: UpdateProductFormProps) {
  const [formData, setFormData] = useState({
    ...
    images:      [] as File[],
    sourceType:  (product as any)?.sourceType  || 'OWN',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    fd.append('amazonUrl',    formData.amazonUrl)
    fd.append('flipkartUrl',  formData.flipkartUrl)
    formData.images.forEach(img => fd.append('images', img))
    onSubmit(fd)
  }

  const isAffiliate = formData.sourceType === 'AMAZON' || formData.sourceType === 'FLIPKART'
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <select id="sourceType" name="sourceType"
              ...
              className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
              <option value="OWN">OWN — sold by MiniGuru (cart + Razorpay)</option>
              <option value="AMAZON">AMAZON — affiliate link (opens Amazon.in)</option>
              <option value="FLIPKART">FLIPKART — affiliate link (opens Flipkart)</option>
            </select>
            <p className="text-xs text-gray-400">
              OWN products go through the MiniGuru cart. Affiliate products open the retailer directly.
            </p>
          </div>

          {/* Affiliate URLs — only shown when AMAZON or FLIPKART */}
          {isAffiliate && (
            <div className="rounded-lg border border-orange-200 bg-orange-50 p-4 space-y-3">
              <p className="text-xs font-semibold text-orange-700">
                🔗 Always include tracking tag: <code>miniguru08-21</code>
              </p>
              <div className="space-y-1">
                <Label htmlFor="amazonUrl">Amazon Affiliate URL</Label>
                <Input id="amazonUrl" name="amazonUrl"
                  value={formData.amazonUrl} onChange={handleChange}
                  placeholder="https://www.amazon.in/dp/B0XXXXXXXX?tag=miniguru08-21" />
                <p className="text-xs text-gray-400">
                  Get ASIN from Amazon product URL — format: amazon.in/dp/[ASIN]?tag=miniguru08-21
                </p>
              </div>
              <div className="space-y-1">
                <Label htmlFor="flipkartUrl">Flipkart URL (optional)</Label>
                <Input id="flipkartUrl" name="flipkartUrl"
                  value={formData.flipkartUrl} onChange={handleChange}
          ...
          <div className="space-y-2">
            ...
            <Input id="images" name="images" type="file" accept="image/*"
              multiple onChange={handleImageChange} className="cursor-pointer" />
            {previews.length > 0 && (
